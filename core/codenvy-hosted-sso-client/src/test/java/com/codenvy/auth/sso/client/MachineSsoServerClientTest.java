...
public class MachineSsoServerClientTest {
    ...
    public void initClient() throws Exception {
        ...
        when(responseMock.asDto(UserDto.class)).thenReturn(newDto(UserDto.class));
    ...
    public void getUserMustDelegateCallToTheSuperWhenTokenIsNotPrefixedWithMachine() throws NotFoundException {
        ssoClient.getUser("123456789", "client", "workspace123", "accountId");
    ...
    public void getUserMustReturnNullWhenTokenIsMachinePrefixedButUserForSuchTokenDoesNotExist() throws NotFoundException {
        final User user = ssoClient.getUser("machine123456789", "client", "workspace123", "accountId");
    ...
    @Test
    public void getUserMustReturnTheUserRetrievedFromTheApiRequest() {
        // machine token
        ...
        final UserDescriptor descriptor = newDto(UserDescriptor.class).withName("name")
                                                                      .withEmail("mail")
                                                                      .withId("user123");
        when(responseMock.asDto(UserDescriptor.class)).thenReturn(descriptor);
        // creating the ticket for figuring out the master's token
        ticketManager.putAccessTicket(new AccessTicket("master_token",
                                                       new UserImpl("user", "user123", "master_token", singleton("user")),
                                                       "auth_type"));

        final User user = ssoClient.getUser(token, "client", "workspace123", "accountId");

        assertNotNull(user);
        assertEquals(user.getName(), descriptor.getName());
        assertEquals(user.getId(), descriptor.getId());
        assertFalse(user.isTemporary());
        assertEquals(user.getToken(), "master_token");
        assertTrue(user.isMemberOf("user"));
    }
}
