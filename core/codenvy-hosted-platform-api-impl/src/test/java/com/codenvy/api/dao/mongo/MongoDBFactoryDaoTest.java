...
public class MongoDBFactoryDaoTest {
    ...
    public void setUp() throws Exception {
        ...
        store = new FactoryDaoImpl(database, COLL_NAME);
    }


    @Test
    public void shouldSaveFactory() throws Exception {
        FactoryDto factory = DtoFactory.getInstance().createDto(FactoryDto.class);
        factory.setV("4.0");
        ...
        FactoryDto result = store.getFactory(id);
        factory.setId(id);
    ...
    public void shouldNotSaveFactoryWithSameNameAndUser() throws Exception {
        ...
        FactoryDto factory1 = DtoFactory.getInstance().createDto(FactoryDto.class)
                                        .withName("testName")
                                        .withCreator(DtoFactory.getInstance().createDto(Author.class)
                                                               .withUserId("userOK"));

        FactoryDto factory2 = DtoFactory.getInstance().createDto(FactoryDto.class)
                                        .withName("testName")
                                        .withCreator(DtoFactory.getInstance().createDto(Author.class)
                                                               .withUserId("userOK"))
    ...
    public void shouldGetFactoryByAttributes() throws Exception {
        ...
        FactoryDto factory3 = DtoFactory.getInstance().createDto(FactoryDto.class)
                                        .withCreator(DtoFactory.getInstance().createDto(Author.class)
                                                               .withUserId("userOK"))
                                        .withWorkspace(DtoFactory.getInstance().createDto(WorkspaceConfigDto.class)
                                                                 .withName("wsName")
        ...
        assertEquals(store.getByAttribute(0, 0, Collections.singletonList(Pair.of("creator.userId", "userOK"))).size(), 3);
        assertEquals(store.getByAttribute(0, 0, Collections.singletonList(Pair.of("workspace.name", "wsName"))).size(), 2);
        assertEquals(store.getByAttribute(0, 0, Arrays.asList(Pair.of("workspace.name", "wsName"),
                                                              Pair.of("workspace.projects.type", "projectType"))).size(), 1);
        assertEquals(store.getByAttribute(0, 0, Arrays.asList(Pair.of("creator.userId", "userOK"),
                                                              Pair.of("workspace.projects.type", "projectType"))).size(), 1);
    }
    ...
    public void shouldUpdateFactory() throws Exception {
        ...
        // new Factory
        FactoryDto updatedFactory = DtoFactory.getInstance().clone(factory);
        ...
        FactoryDto result = DtoFactory.getInstance().createDtoFromJson(JSON.serialize(res), FactoryDto.class);
    ...
    public void shouldNotUpdateFactoryWithConflictingName() throws Exception {
        ...
        FactoryDto originalFactory = DtoFactory.getInstance().createDto(FactoryDto.class);
...
;
        FactoryDto updatedFactory = DtoFactory.getInstance().clone(originalFactory);
    ...
    public void shouldEncodedDot() {
        ...
        assertEquals(encoded, "hello" + FactoryDaoImpl.ESCAPED_DOT + "my" + FactoryDaoImpl.ESCAPED_DOT + "string");
    ...
    public void shouldEncodedDollar() {
        String original = "hello$my$string";
        String encoded = store.encode(original);
        assertEquals("hello" + FactoryDaoImpl.ESCAPED_DOLLAR + "my" + FactoryDaoImpl.ESCAPED_DOLLAR + "string", encoded);
        String decoded = store.decode(encoded);
        assertEquals(original, decoded);
    }
    ...
    public void shouldEncodedDotDollar() {
        String original = "hello.my$string";
        String encoded = store.encode(original);
        assertEquals("hello" + FactoryDaoImpl.ESCAPED_DOT + "my" + FactoryDaoImpl.ESCAPED_DOLLAR + "string", encoded);
        String decoded = store.decode(encoded);
        assertEquals(original, decoded);
    }
