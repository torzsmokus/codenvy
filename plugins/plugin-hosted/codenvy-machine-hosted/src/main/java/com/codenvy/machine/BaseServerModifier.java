...
public abstract class BaseServerModifier implements MachineServerModifier {
    ...
    public ServerImpl proxy(ServerImpl server) {
        final int colonIndex = server.getAddress().indexOf(':');
        final String serverHost = server.getAddress().substring(0, colonIndex);
        final String serverPort = server.getAddress().substring(colonIndex + 1);
